        ...
        if IsPlayerDead(PlayerId()) then
            ...
            while not pressKey and setDead do
                ...
                if not IsEntityAttachedToAnyPed(PlayerPedId()) then
                    ...
                    NetworkSetInSpectatorMode(false, PlayerPedId())
                    DrawText(Config.Langs["SubTitlePressKey"], Config["RespawnSubTitleFont"], 0.50, 0.20, 1.6, 1.6, 255, 255, 255, 255, true, true)
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif TimeToRespawn >= 0 and setDead then
			ProcessCamControls()
            NetworkSetInSpectatorMode(false, PlayerPedId())
            DrawText(Config.Langs["TitleOnDead"], Config["RespawnTitleFont"], 0.50, 0.15, 3.5, 3.5, 171, 3, 0, 255, true, true)
            DrawText(string.format(Config.Langs["SubTitleOnDead"], TimeToRespawn), Config["RespawnSubTitleFont"], 0.50, 0.35, 0.5, 0.5, 255, 255, 255, 255, true, true)
        else
            Citizen.Wait(500)
        end
    end
...
function StartDeathCam()
    Citizen.CreateThread(function()
        ...
        cam = CreateCamWithParams("DEFAULT_SCRIPTED_CAMERA", coords, 0.00, 0.00, 0.00, camHeight*1.0, false, 0)
        AttachCamToPedBone(cam, playerPed, 0, 2.0, 2.0, camHeight*1.0, 0)
        PointCamAtEntity(cam, playerPed, 1, 1, 0, true)
        SetCamActive(cam, true)
        RenderScriptCams(true, true, 200, true, false)
        PointCamAtEntity(cam, playerPed, 1, 1, 0, false)
    end)
end
